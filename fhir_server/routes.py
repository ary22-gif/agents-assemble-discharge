...
def _fhir_json(data: dict, status: int = 200) -> JSONResponse:
    return JSONResponse(
        content=data, status_code=status, media_type="application/fhir+json"
    )
...
def _search_handler(
    resource_type: str, patient_id: str, authorization: Optional[str]
) -> JSONResponse:
    _require_auth(authorization)
    ...
    logger.info(
        "FHIR GET %s?patient=%s → %d results", resource_type, patient_id, len(resources)
    )
...
def search_conditions(
    patient: str = Query(...), authorization: Optional[str] = Header(None)
):
...
def search_medication_statements(
    patient: str = Query(...), authorization: Optional[str] = Header(None)
):
