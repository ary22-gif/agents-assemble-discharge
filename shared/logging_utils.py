...
class AuditLogger:
    ...
    def log_call(self, patient_id: str, tool_name: str, duration_ms: float, resource_ids: list[str]):
        self._log.info(
            "AUDIT agent=%s patient_id=%s tool=%s duration_ms=%.1f resources=%s",
            self._agent, patient_id, tool_name, duration_ms, resource_ids,
        )
    ...
    def log_task(self, task_id: str, patient_id: str, duration_ms: float, status: str):
        self._log.info(
            ...
            self._agent, task_id, patient_id, duration_ms, status,
        )
