...
FHIR_EXT  = os.getenv("FHIR_EXTENSION_URI", "https://app.promptopinion.ai/schemas/a2a/v1/fhir-context")
...
def main():
    ...
    try:
        ...
        for skill in card.get("skills", []):
            console.print(f"  skill: [bold]{skill.get('name')}[/] — {skill.get('description', '')[:80]}")
    ...
    payload = {
        ...
        "params": {
            "message": {
                "role": "user",
                "parts": [
                    {"kind": "text", "text": f"Prepare discharge packet for patient {PATIENT}."}
                ],
                "metadata": {
                    FHIR_EXT: {
                        "fhirUrl":   FHIR_URL,
    ...
    console.print(Syntax(json.dumps(payload, indent=2)[:600] + "\n  ...", "json", theme="monokai"))
    ...
    console.print(f"[green]✓ Response received in {elapsed:.1f}s (HTTP {resp.status_code})[/]")
    ...
    try:
        ...
        console.print(f"[yellow]Note: Response is not JSON. Raw text:[/]\n{text[:1000]}")
        sys.exit(0)
    ...
    console.print(f"[bold]Duration:[/] {packet.get('total_duration_ms', '?')}ms (agent-reported)")

    for section, label in [("medications", "MedRecon"), ("care_instructions", "CarePlan"), ("follow_up", "FollowUp")]:
        s = packet.get(section, {})
        status_str = s.get("status", "missing")
        color = "green" if status_str == "success" else "red"
        console.print(f"  [{color}]{'✓' if status_str == 'success' else '✗'}[/{color}] {label}: {status_str}")
